...
import utils
from purchase_invoice import SupplierItem, PurchaseInvoice
from settings import NKK_ACCOUNTS, KORNKRAFT_ACCOUNTS


class InvoiceIntermediateModel:
    def __init__(self, purchase_invoice, generation_type, lines):
    ...
    def set_basic_info(self):
        ...
        elif self.generation_type == 'kornkraft':
            for vat in self.purchase_invoice.vat_rates:
                    ...
                    if len(words) > 12:
                        ...
                        for vat in vat_rate_strs:
                            if vat in words[0:6]:
                                ...
                                break
        elif self.generation_type == 'krannich':
            for line in self.lines:
                if line[0].isdigit():
            ...
            if not self.purchase_invoice.order_id:
                for i in range(-1, -len(self.line_items) - 1, -1):
                    ...
                    if order_id_lines:
                        ...
                        break
        elif self.generation_type == 'pvxchange':
            preamble = True
            for line in self.lines:
                words = line.split()
                for i in [1, 2]:
                ...
                if pos_no >= 0 or 'Nettosumme' in line:
                    ...
                    item.append(line)
            self.line_items.append(item)
        elif self.generation_type == 'heckert':
            for line in self.lines:
                words = line.split()
                ...
                if words and words[0] and words[0][0].isdigit():
                    ...
                    item.append(line)
            self.line_items.append(item)
        elif self.generation_type == 'wagner':
            for line in self.lines:
                words = line.split()
        ...
        if self.generation_type == 'nkk':
            ...
            for i in range(-1, -len(self.line_items) - 1, -1):
                ...
                if vat_lines:
                    ...
                    if self.purchase_invoice.update_stock:
                        self.purchase_invoice.shipping = PurchaseInvoice.get_amount_krannich \
            ...
            self.purchase_invoice.vat[self.purchase_invoice.default_vat] = PurchaseInvoice.get_amount_krannich(
                [vat_line])
        elif self.generation_type == 'pvxchange':
