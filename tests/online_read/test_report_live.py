...
class TestFinancialStatements:
    ...
    def test_build_report(self, live: LiveState, user_settings: UserSettings, kwargs: dict[str, Any]) -> None:
...
class TestGeneralLedger:
    def test_get_gl_and_total(self, live: LiveState) -> None:
    ...
    def test_balance(self, live: LiveState) -> None:
        year = datetime.date.today().year
        acc = live.bank_leaf()
    ...
    def test_income_pretax_declaration(self, live: LiveState, capsys: pytest.CaptureFixture[str]) -> None:
        import settings
        import utils
...
class TestTables:
    def test_projects(self, live: LiveState) -> None:
        tbl = report.projects()
        assert tbl.title == "Projekte"
        for e in tbl.entries:
            assert e["Marge"] == pytest.approx(e["Verkauf"] - e["Einkauf"])

    def test_sold_items_of_first_project(self, live: LiveState, api: Any) -> None:
    ...
    def test_opportunities(self, live: LiveState) -> None:
        tbl = report.opportunities(live.company_name)
        assert isinstance(tbl, table.Table)

    def test_balkonmodule_month(self, live: LiveState, monkeypatch: pytest.MonkeyPatch) -> None:
        year = datetime.date.today().year
    ...
    def test_balances_uses_plotly(self, live: LiveState, monkeypatch: pytest.MonkeyPatch) -> None:
        import settings
