...
class TestFunctionWrapper:
    def test_returns_resource_and_captures_output(self):
    ...
    def test_exception_is_captured_with_traceback(self):
...
class TestApiWrapper:
    def test_clean_call_has_no_error(self):
        r = api_wrapper.api_wrapper(ok, 1)
        assert r["err_msg"] == ""
        assert r["resource"] == 2

    def test_error_lines_are_extracted_from_html(self):
        def f():
    ...
    def test_raise_exception_line_is_used_when_no_error_line(self):
        def f():
            print("<p>a\n    raise raise_exception(msg)\nDie eigentliche Meldung\nb</p>")
        r = api_wrapper.api_wrapper(f)
        assert r["err_msg"] == "Die eigentliche Meldung"

    def test_fallback_to_head_and_tail(self):
        def f():
    ...
    def test_plain_output_counts_as_error_message(self):
    ...
    def test_whitespace_only_output_is_no_error(self):
        def f():
            print("   \n  ")
...
class TestApiWrapperTest:
    ...
    def test_false_on_exception_or_output(self):
...
class TestGuiApiWrapper:
    ...
    def test_returns_none_and_reports_on_exception(self, capsys):
    ...
    def test_treats_stdout_as_error(self, capsys):
