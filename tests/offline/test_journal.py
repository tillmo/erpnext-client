...
def gl_handler(balances, rows=None):
...
class TestJournalEntry:
    def test_mirrored_accounts(self, somiko, fake_api, capsys):
    ...
    def test_cheque_no(self, somiko, fake_api):
    ...
    def test_journal_entry3_positive(self, somiko, fake_api):
    ...
    def test_journal_entry3_negative(self, somiko, fake_api):
        j = journal.journal_entry3(somiko, "K", "G1", "G2", -1000.0, -190.0, "t", "r", "2026-01-01")
        accs = strip_meta(j["accounts"])
...
class TestAddParty:
    def test_string_account_unchanged(self):
        e = {"account": "Bank", "debit": 1}
        assert journal.add_party_acc(e) is e

    def test_dict_account_adds_party(self):
    ...
    def test_reference_to_journal_entry(self, ptype, debit, credit, expect_ref):
...
class TestGeneralLedger:
    def test_get_gl_passes_filters(self, fake_api):
...
class TestTaxJournalEntries:
    def test_creates_vorsteuer_and_umsatzsteuer(self, somiko, fake_api, capsys):
    ...
    def test_zero_balances_create_nothing(self, somiko, fake_api, capsys):
    ...
    def test_unknown_company(self, fake_api, capsys):
...
class TestIncomeDistribution:
    def test_distribution_by_expense_ratio(self, laden, fake_api, capsys):
...
def add_advance_payment(fake_api, somiko, references=()):
...
@pytest.fixture
def advance_payment(somiko, fake_api):
    return add_advance_payment(fake_api, somiko)


@pytest.fixture
def advance_payment_with_invoice(somiko, fake_api):
...
class TestAdvancePaymentJournalEntry:
    def test_umbuchung_pay(self, somiko, fake_api, advance_payment, capsys):
    ...
    def test_umbuchung_is_idempotent(self, somiko, fake_api, advance_payment, capsys):
        journal.create_advance_payment_journal_entry(advance_payment, 19)
        journal.create_advance_payment_journal_entry(advance_payment, 19)
        assert len(fake_api.get_list("Journal Entry")) == 1
        assert "existiert schon" in capsys.readouterr().out

    def test_umbuchung_receive(self, somiko, fake_api):
    ...
    def test_rueckbuchung(self, somiko, fake_api, advance_payment_with_invoice):
    ...
    def test_rueckbuchung_without_invoice(self, somiko, fake_api, advance_payment, capsys):
    ...
    def test_rueckbuchung_without_umbuchung(self, somiko, fake_api, advance_payment_with_invoice, capsys):
...
class TestAdvancePaymentJournalEntries:
    def _seed(self, fake_api, somiko):
    ...
    def test_year_run(self, somiko, fake_api):
...
class TestVatFunctions:
    def test_income_and_pretax(self, somiko, fake_api):
    ...
    def test_save_pretax_details(self, somiko, fake_api, in_tmp_cwd):
        acc = TAX_ACCOUNTS[somiko.name]["pre_tax_accounts"][0]
    ...
    def test_vat_declaration_prints_summary(self, somiko, fake_api, capsys):
        fake_api.add("Company", **F.company_doc())
    ...
    def test_save_purchase_invoices(self, somiko, fake_api, in_tmp_cwd, user_settings, capsys):
