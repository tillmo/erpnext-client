...
@pytest.fixture
def catalogue(somiko: Company, fake_api: FakeFrappeClient) -> FakeFrappeClient:
...
@pytest.fixture
def s_item(somiko: Company, catalogue: FakeFrappeClient) -> SupplierItem:
...
class TestSearchItem:
    def test_translation_table_hit(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        s_item.item_code = "KS-400"
        assert s_item.search_item(SUPPLIER)["item_code"] == "010.100.001"
        assert gui.calls == [] and catalogue.calls == []

    def test_choice_from_similar_items(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
    ...
    def test_choice_with_supplier_code_learns_translation(self, s_item: SupplierItem, catalogue: FakeFrappeClient,
    ...
    def test_cancel(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        gui.answers["choicebox"] = None
        assert s_item.search_item(SUPPLIER) is None

    def test_new_item_via_gui(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
    ...
    def test_new_item_declined(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        answers = iter(["Neuen Artikel anlegen", "Wechselrichter", "Nein"])
        gui.answers["choicebox"] = lambda msg, title, texts: next(answers)
        assert s_item.search_item(SUPPLIER) is None
        assert catalogue.calls_of("insert") == []

    def test_new_item_group_cancelled(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        answers = iter(["Neuen Artikel anlegen", None])
        gui.answers["choicebox"] = lambda msg, title, texts: next(answers)
        assert s_item.search_item(SUPPLIER) is None

    def test_new_item_without_dialogs(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        e_item = s_item.search_item(SUPPLIER, check_dup=False)
        assert e_item["item_group"] == settings.STANDARD_ITEM_GROUP
        assert gui.calls == []

    def test_unanswered_dialog_detected(self, s_item: SupplierItem, catalogue: FakeFrappeClient) -> None:
        with pytest.raises(GuiCalled):
            s_item.search_item(SUPPLIER)


class TestAddItemPrice:
    def test_creates_price(self, s_item: SupplierItem, catalogue: FakeFrappeClient) -> None:
    ...
    def test_same_price_untouched(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        catalogue.add("Item Price", item_code="010.100.001", price_list_rate=150.0)
        s_item.add_item_price(MODUL, 150.0, "Stk", "2026-01-01")
        assert catalogue.calls_of("update") == [] and gui.calls == []

    def test_changed_price_asks(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
...
class TestProcessItem:
    def test_stock_item(self, s_item: SupplierItem, catalogue: FakeFrappeClient) -> None:
        s_item.item_code = "KS-400"
    ...
    def test_non_stock_item_becomes_aggregate(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
    ...
    def test_elektro_group_aggregate(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
    ...
    def test_no_item_found(self, s_item: SupplierItem, catalogue: FakeFrappeClient, gui: EasyguiStub) -> None:
        gui.answers["choicebox"] = None
