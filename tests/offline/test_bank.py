...
@pytest.fixture
def bacc(somiko, fake_api):
    return F.make_bank_account(fake_api, somiko)


def add_bt(fake_api, bacc, **kw):
...
class TestBankAccount:
    def test_init_registers_and_computes_balance(self, somiko, fake_api):
    ...
    def test_init_baccounts_from_server(self, somiko, fake_api, user_settings):
    ...
    def test_init_baccounts_skipped_during_setup(self, fake_api, user_settings):
        user_settings["-setup-"] = True
        fake_api.add("Bank Account", **F.bank_account_doc())
        bank.BankAccount.init_baccounts()
        assert bank.BankAccount.baccounts_by_name == {}

    def test_clear(self, bacc):
...
class TestBankTransactionBasics:
    def test_init_deposit(self, bacc, fake_api):
    ...
    def test_init_withdrawal_without_description(self, bacc, fake_api):
        doc = add_bt(fake_api, bacc, withdrawal=25.5)
        del doc["description"]
        bt = bank.BankTransaction(doc)
        assert bt.amount == -25.5 and bt.description == ""

    def test_link_to(self, bacc, fake_api):
...
class TestBankTransactionBooking:
    def test_journal_entry_against_account(self, bacc, fake_api, user_settings):
    ...
    def test_journal_entry_withdrawal_with_buchen(self, bacc, fake_api, user_settings, capsys):
    ...
    def test_journal_entry_between_bank_accounts(self, bacc, fake_api, somiko):
    ...
    def test_payment_for_invoice_limits_to_outstanding(self, bacc, fake_api, somiko):
    ...
    def test_advance_payment_without_invoice(self, bacc, fake_api):
    ...
    def test_payment_failure_returns_none(self, bacc, fake_api, monkeypatch):
        doc = add_bt(fake_api, bacc, deposit=100.0)
...
class TestTransfer:
    def _open_invoices(self, fake_api, somiko):
    ...
    def test_options_offered(self, bacc, fake_api, somiko, gui):
    ...
    def test_choose_account_creates_journal_entry(self, bacc, fake_api, somiko, gui):
        sinvs, pinvs = self._open_invoices(fake_api, somiko)
    ...
    def test_choose_invoice_creates_payment(self, bacc, fake_api, somiko, gui):
        sinvs, pinvs = self._open_invoices(fake_api, somiko)
    ...
    def test_choose_advance(self, bacc, fake_api, somiko, gui):
    ...
    def test_choose_other_bank_transaction(self, bacc, fake_api, somiko, gui):
        other = F.make_bank_account(fake_api, somiko, name="Sparda - SoMiKo", iban=F.IBAN_SPARDA, account="Bank2 - SoMiKo")
    ...
    def test_cancel_does_nothing(self, bacc, fake_api, somiko, gui):
        doc = add_bt(fake_api, bacc, deposit=100.0)
        gui.answers["choicebox"] = None
        bank.BankTransaction(doc).transfer([], [])
        assert fake_api.calls_of("insert") == [] and fake_api.calls_of("update") == []

    def test_unanswered_dialog_is_detected(self, bacc, fake_api):
        doc = add_bt(fake_api, bacc, deposit=100.0)
...
class TestSubmitAndDelete:
    def _linked(self, bacc, fake_api):
        doc = add_bt(fake_api, bacc, deposit=100.0, description="Spende")
        bt = bank.BankTransaction(doc)
        bt.journal_entry("8401 - Selbstbauanlagen 19% - SoMiKo", False)
    ...
    def test_submit_entry_submits_bank_transaction_and_journal(self, bacc, fake_api, capsys):
    ...
    def test_submit_entry_leaves_partially_allocated_bt_open(self, bacc, fake_api):
        doc = add_bt(fake_api, bacc, deposit=100.0)
        bt = bank.BankTransaction(doc)
    ...
    def test_delete_entry_resets_bank_transaction(self, bacc, fake_api, capsys):
        bt_name, je = self._linked(bacc, fake_api)
    ...
    def test_delete_entry_without_bank_transaction(self, bacc, fake_api, capsys):
        je = fake_api.add("Journal Entry", accounts=[])
        bank.BankTransaction.delete_entry(je)
        assert fake_api.get_list("Journal Entry") == []
        assert "nicht in Banktransaktionen gefunden" in capsys.readouterr().out

    def test_delete_entry_payment(self, bacc, fake_api, somiko):
        doc = add_bt(fake_api, bacc, deposit=100.0)
    ...
    def test_cancelled_entry_only_resets(self, bacc, fake_api):
        bt_name, je = self._linked(bacc, fake_api)
    ...
    def test_cancelled_entry_with_amendment_is_skipped(self, bacc, fake_api):
        bt_name, je = self._linked(bacc, fake_api)
        fake_api.cancel("Journal Entry", je)
    ...
    def test_unreconcile_for_cancelled_links(self, bacc, fake_api, somiko):
        bt_name, je = self._linked(bacc, fake_api)
...
class TestFindBankTransaction:
    def test_finds_unique_match(self, bacc, fake_api, somiko):
    ...
    def test_deposit_for_positive_total(self, bacc, fake_api, somiko):
    ...
    def test_ambiguous_or_missing(self, bacc, fake_api, somiko):
    ...
    def test_reconciled_transactions_are_ignored(self, bacc, fake_api, somiko):
        add_bt(fake_api, bacc, withdrawal=10.0, description="RE-1", status="Reconciled")
        assert bank.BankTransaction.find_bank_transaction(somiko.name, -10.0, "RE-1") is None


class TestReconcilePreInvoice:
    def _pre(self, fake_api, somiko, pi_status="Unpaid", docstatus=1):
    ...
    def test_pays_linked_purchase_invoice(self, bacc, fake_api, somiko, capsys):
    ...
    def test_cancelled_invoice(self, bacc, fake_api, somiko, capsys):
    ...
    def test_missing_pre_invoice_and_link(self, bacc, fake_api, somiko, capsys):
    ...
    def test_reconcile_pre_invoices_selects_pre_descriptions(self, bacc, fake_api, somiko):
        pi = self._pre(fake_api, somiko)
...
class TestBankStatementEntry:
    def test_bank_transaction_dict(self, bacc):
...
class TestBankStatementReaders:
    def test_read_sparkasse_bremen(self, bacc, tmp_path):
    ...
    def test_read_sparda_ethik(self, bacc, tmp_path):
    ...
    def test_get_baccount_from_iban_column(self, bacc, tmp_path):
        fn = F.write_sparkasse_csv(tmp_path / "spk.csv", ROWS_SPARKASSE)
        assert bank.BankStatement.get_baccount(fn) == (bacc, F.IBAN_SPARKASSE)

    def test_get_baccount_from_second_column(self, bacc, somiko, fake_api, tmp_path):
    ...
    def test_get_baccount_from_blz_and_konto(self, somiko, fake_api, tmp_path):
    ...
    def test_get_baccount_unknown(self, bacc, tmp_path):
    ...
    def test_read_statement_dispatch(self, somiko, fake_api, tmp_path, gui):
    ...
    def test_read_statement_unknown_account(self, bacc, tmp_path, gui):
        gui.answers["msgbox"] = None
...
class TestProcessFile:
    def test_imports_new_transactions_and_deduplicates(self, bacc, fake_api, tmp_path):
        fn = F.write_sparkasse_csv(tmp_path / "spk.csv", ROWS_SPARKASSE)
    ...
    def test_cancelled_duplicate_is_reimported(self, bacc, fake_api, tmp_path):
    ...
    def test_sparda_sets_statement_balance(self, somiko, fake_api, tmp_path):
        sparda = F.make_bank_account(fake_api, somiko, name="Sparda - SoMiKo", iban=F.IBAN_SPARDA)
    ...
    def test_unknown_file_returns_none(self, bacc, tmp_path, gui):
        gui.answers["msgbox"] = None
