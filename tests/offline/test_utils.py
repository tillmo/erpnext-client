...
class TestDates:
    ...
    def test_last_quarter(self, d, expected):
    ...
    def test_quarter_to_dates(self, quarter, expected):
        assert utils.quarter_to_dates(quarter) == expected

    def test_last_quarter_and_quarter_to_dates_are_consistent(self):
    ...
    @requires_de_locale
    def test_convert_date_written_month(self, restore_locale):
...
class TestStrings:
    ...
    def test_showlist_skips_empty(self):
    ...
    def test_find_ref(self, line, expected):
        assert utils.find_ref(line) == expected

    def test_extract_prnr(self):
...
class TestFormatting:
    ...
    def test_to_str_date_and_passthrough(self):
    ...
    def test_print_dicts(self, capsys):
    ...
    def test_title(self, user_settings):
...
class TestIban:
    def test_iban_de_known_example(self):
...
class TestFiles:
    ...
    def test_get_csv_plain(self, tmp_path):
        p = tmp_path / "a.csv"
        p.write_text("a;b\r\nc\nd;e\r\n", encoding="utf-8")
        rows = list(utils.get_csv("utf-8", str(p)))
        assert rows == [["a", "b"], ["c"], ["d", "e"]]

    def test_get_csv_replacenl_joins_embedded_newlines(self, tmp_path):
        p = tmp_path / "a.csv"
        p.write_text("a;b\r\nc\nd;e\r\n", encoding="utf-8")
        rows = list(utils.get_csv("utf-8", str(p), replacenl=True))
        assert rows == [["a", "b"], ["c d", "e"], []]

    def test_get_csv_codec(self, tmp_path):
        p = tmp_path / "latin.csv"
        p.write_bytes("Müller;1\n".encode("iso-8859-4"))
        assert list(utils.get_csv("iso-8859-4", str(p))) == [["Müller", "1"]]

    def test_evince_spawns_process(self, monkeypatch):
        calls = []
